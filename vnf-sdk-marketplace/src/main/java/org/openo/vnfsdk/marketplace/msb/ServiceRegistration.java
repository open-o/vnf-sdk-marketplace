...
public class ServiceRegistration implements Runnable {
    ...
    public void run() 
    {
        ...
        while(!flag) 
        {
            LOG.info("VNF-SDK MarketPlace test microservice register.retry:" + retry);
            retry++;
        ...
        LOG.info("VNF-SDK MarketPlace test microservice register end.");
    }
    ...
    private void initServiceEntity() {
        funcTestEntity.setServiceName("vnf-sdk");
        funcTestEntity.setProtocol("REST");
        funcTestEntity.setVersion("v1");
        funcTestEntity.setUrl("/*");
        funcTestEntity.setSingleNode(Config.getConfigration().getServiceIp(), "8702", 0);
